...
namespace nest_mn {
  ...
  inline MPI_Comm communicator(nest_mn::Setup* s) {
    return (MPI_Comm) s->communicator();
  }

  inline MPI_Comm communicator(nest_mn::Runtime* r) {
    return (MPI_Comm) r->communicator();
  }

  inline int toint(nest_mn::GlobalIndex i) {
    return i;
  }

  inline int toint(nest_mn::LocalIndex i) {
    return i;
  }
