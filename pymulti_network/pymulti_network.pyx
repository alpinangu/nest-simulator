...
cdef class MessageOutputPort(Port):
    ...
    def insertMessage(self, double time, object msg):
        ...
        cdef bytearray pmsg
        if self.pickled:
            pmsg = bytearray(pickle.dumps(msg, pickle.HIGHEST_PROTOCOL))
        else:
            # accept bytes/bytearray/memoryview/etc.
            pmsg = msg if isinstance(msg, bytearray) else bytearray(msg)
...
cdef class Setup(object):
    ...
    def __cinit__(self, list argv=None, required=None):
        ...
        try:
            ...
            self.argv = [r.argv[i] for i in range(r.argc)]
        finally:
            free(r.argv)
