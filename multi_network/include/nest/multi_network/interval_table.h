...
#define VANILLA   0 // intervals stored in a linked list in one
			  // contiguous memory block

#define TRIMMED   1 // linked list block trimmed during build

#define COMPACT   2 // linked list block freed during build and
			  // replaced by a single array of intervals

#define SCATTERED 3 // list block freed during build and
			  // replaced by many small arrays of
			  // intervals---one per key in the table


#ifndef ITABLE_FLAVOR
#define ITABLE_FLAVOR VANILLA
#endif

//#define ITABLE_COMPRESSED
...
namespace nest_mn {
  ...
  private:
    ...
    class Entry {
#ifdef ITABLE_COMPRESSED
      PointType key_;
#endif
#if ITABLE_FLAVOR == VANILLA || ITABLE_FLAVOR == TRIMMED
      IList ls_;
    public:
#ifdef ITABLE_COMPRESSED
...
#endif
#if ITABLE_FLAVOR == COMPACT || ITABLE_FLAVOR == SCATTERED
    IList::Interval* iPtr;
#if ITABLE_FLAVOR == COMPACT
...
#endif
#else // COMPACT or SCATTERED
	int n = list.size ();
#if ITABLE_FLAVOR == SCATTERED
...
#endif
    delete preEntryMap_;
#if ITABLE_FLAVOR == TRIMMED
    IList::trimNodes ();
#elif ITABLE_FLAVOR == COMPACT || ITABLE_FLAVOR == SCATTERED
    IList::reset ();
#endif
